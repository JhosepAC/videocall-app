...
const CursorGrid = ({
    ...
    const autoPulsedRef = useRef(false);
    ...
    useEffect(() => {
        ...
        if (propsRef.current.clickPulse && autoPulse && !autoPulsedRef.current) {
            autoPulsedRef.current = true;
            pulses.push({x: w / 2, y: h / 2, t0: performance.now()});
            wake();
        }
